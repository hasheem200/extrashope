...
/*
  ==============================================================
  Sends a professional-looking copy of a platform notification to
  the admin's configured "Notification Email" (Admin -> Payment
  Settings -> Notification Email).

  This is intentionally "fire and forget" from the caller's point
  of view: if the email fails (or no notification email is set),
  it never throws and never blocks whatever real action triggered
  it (a withdraw request still succeeds even if this email fails).
  ==============================================================
*/
...
async function notifyAdmin({ type = "info", title, message, details = {}, actionUrl, actionLabel }) {

    try {

        const settings = await Settings.findOne();

        const notificationEmail = settings?.paymentSettings?.notificationEmail;

        if (!notificationEmail || notificationEmail.trim() === "") {
            // no admin notification email configured — nothing to do,
...
<div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;background:#f5f5f5;padding:24px;">
  ...
  <div style="background:#111;color:white;padding:20px 24px;border-radius:12px 12px 0 0;">
    ...
    <div style="font-size:13px;color:#aaa;margin-top:2px;">Platform Notification</div>
  </div>
  ...
  <div style="background:white;padding:24px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,.06);">
    ...
    <p style="color:#999;font-size:12px;margin-top:24px;border-top:1px solid #eee;padding-top:14px;">
      Sent automatically by ${siteName}${siteUrl ? ` — <a href="${siteUrl}" style="color:#999;">${siteUrl.replace(/^https?:\/\//, "")}</a>` : ""} at ${new Date().toLocaleString()}.
        ...
        console.log(`✅ Admin notification email sent: "${title}" -> ${notificationEmail}`);

    } catch (err) {

        // Never let a notification-email failure break the real
        // action that triggered it.
        console.log(`⚠️  Admin notification email failed ("${title}"):`, err.message);
